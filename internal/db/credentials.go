...
func (c *Client) ListCredentials(ctx context.Context) ([]Credential, error) {
	query := `
		SELECT id, name, type, data, "nodesAccess", "createdAt", "updatedAt"
	...
	for rows.Next() {
		var cred Credential
		var nodesAccess []byte
		err := rows.Scan(
			&cred.ID, &cred.Name, &cred.Type, &cred.Data,
			&nodesAccess, &cred.CreatedAt, &cred.UpdatedAt,
		)
		if err != nil {
			return nil, fmt.Errorf("scanning credential row: %w", err)
		}
		cred.NodesAccess = nodesAccess
...
func (c *Client) UpsertCredential(ctx context.Context, cred Credential) error {
	query := `
		...
		ON CONFLICT (id) DO UPDATE SET
			...
			type = EXCLUDED.type,
			data = EXCLUDED.data,
			"nodesAccess" = EXCLUDED."nodesAccess",
			"updatedAt" = CURRENT_TIMESTAMP
	`

	// Default nodesAccess to empty array if nil
	var nodesAccessStr string
	if len(cred.NodesAccess) > 0 {
		nodesAccessStr = string(cred.NodesAccess)
	} else {
		nodesAccessStr = "[]"
	}

	_, err := c.pool.Exec(ctx, query,
		cred.ID, cred.Name, cred.Type, cred.Data, nodesAccessStr,
	)
	return err
}
